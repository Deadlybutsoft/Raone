...
// FIX: Add global type declarations for Inkeep to satisfy TypeScript.
// This informs the compiler that these properties will be available on the window object at runtime.
interface InkeepChatButtonInstance {
  open: () => void;
  cleanup: () => void;
}
...
declare global {
  interface Window {
    Inkeep?: {
      ChatButton: (config: any) => InkeepChatButtonInstance;
    };
    inkeep?: {
      open: () => void;
    };
...
const InkeepChatButton: React.FC = () => {
  useEffect(() => {
    let chatButton: InkeepChatButtonInstance | null = null;
      ...
      if (window.Inkeep?.ChatButton) {
        ...
        const config = {
          baseSettings: {
            ...
            organizationDisplayName: "Raone",
            primaryBrandColor: "#FBBF24", // amber-400
          },
          aiChatSettings: {
            aiAssistantName: "Raone Assistant",
             botAvatar: {
